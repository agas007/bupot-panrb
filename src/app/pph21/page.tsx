...
type RecipientPtkpRow = { id: number; nik: string; name: string; taxYear: number; statusPtkp: string; terCategory: "A" | "B" | "C" | null; category: string | null; sourceData: string | null; note: string | null };
type Recipient = {
  id: number;
  nik: string;
  name: string;
  ...
  totalGross: number;
  totalTax: number;
  exportedGross: number;
  exportedTax: number;
  monthlySummary: Array<{ period: string; count: number; gross: number; tax: number }>;
  transactions: Array<{ id: number; spmNumber: string; sp2dNumber: string | null; sp2dDate: string | null; status: string; taxObjectCode: string; gross: number; calculatedTax: number }>;
  ptkpLatest?: RecipientPtkpRow | null;
  ptkpHistory?: RecipientPtkpRow[];
};
...
type PtkpImportRow = { rowNumber: number; nik: string; name: string; statusPtkp: string; taxYear: number; category: string; sourceData: string; note: string; status: "VALID" | "DUPLICATE" | "INVALID"; message?: string };
type PtkpImportReport = { fileName: string; totalRows: number; importedCount: number; duplicateCount: number; invalidCount: number; previewOnly: boolean; rows: PtkpImportRow[] };
...
export default function Pph21Page() {
  ...
  const [isImportingPtkp, setIsImportingPtkp] = useState(false);
  ...
  async function checkImportedPtkp(file: File) {
    setBusy(true);
    setIsImportingPtkp(true);
    setFeedback(null);
    setPtkpImportReport(null);
    try {
      const formData = new FormData();
      formData.append("file", file);
      const res = await fetch("/api/pph21/recipients/import", {
        method: "POST",
        headers: getAuthHeaders(),
        body: formData,
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || "Gagal mengimpor master PTKP");
      setPtkpImportReport(data);
      await loadRecipients();
      setFeedback({ type: data.invalidCount ? "error" : "success", message: `${data.importedCount} baris master PTKP diproses, ${data.duplicateCount} duplikat, ${data.invalidCount} invalid.` });
    } catch (error) {
      setFeedback({ type: "error", message: error instanceof Error ? error.message : "Gagal mengimpor master PTKP" });
    } finally {
      setBusy(false);
      setIsImportingPtkp(false);
    }
  }
  ...
  return <div className="flex flex-col gap-6 pb-10 text-left">
    <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
      ...
      <div className="flex w-full flex-col gap-2 sm:flex-row sm:flex-wrap sm:justify-end lg:w-auto">
        {isAdmin && (
          <label className={`w-full sm:w-auto px-5 py-3 rounded-xl border border-violet-500/30 text-violet-600 font-bold flex items-center justify-center gap-2 cursor-pointer hover:bg-violet-500/10 ${isImportingPtkp ? "opacity-80 pointer-events-none" : ""}`}>
            {isImportingPtkp ? <Loader2 size={17} className="animate-spin" /> : <Upload size={17} />}
            {isImportingPtkp ? (isID ? "Mengimpor master PTKP..." : "Importing PTKP master...") : (isID ? "Impor Master PTKP" : "Import PTKP Master")}
            <input type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" className="hidden" disabled={busy} onChange={(event) => { const file = event.target.files?.[0]; if (file) void checkImportedPtkp(file); event.target.value = ""; }}/>
          </label>
        )}
        <label className={`w-full sm:w-auto px-5 py-3 rounded-xl border border-accent/30 text-accent font-bold flex items-center justify-center gap-2 cursor-pointer hover:bg-accent/10 ${isImportingXml ? "opacity-80 pointer-events-none" : ""}`}>
          ...
          {isImportingXml ? (isID ? "Mengimpor XML..." : "Importing XML...") : (isID ? "Impor & Cek XML" : "Import & Check XML")}
        ...
        <button onClick={exportXml} disabled={busy || !selected.size} className="w-full sm:w-auto premium-button px-5 py-3 flex items-center justify-center gap-2 disabled:opacity-40">
          <Download size={17} /> {isID ? `Ekspor ${selected.size} SP2D ke XML` : `Export ${selected.size} SP2D to XML`}
        </button>
      </div>
    </header>
    ...
    {importReport && <section className="glass-card p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="font-black text-lg">{isID ? "Hasil impor dan pengecekan XML" : "XML import and validation results"}</h2><p className="text-sm text-muted-foreground">{importReport.fileName} · {importReport.totalRows} {isID ? "recipient dalam" : "recipients in"} {importReport.totalDocuments} SP2D</p></div><button onClick={() => setImportReport(null)}><X size={20}/></button></div><div className="grid grid-cols-3 gap-3 mt-4"><div className="rounded-xl bg-emerald-500/10 text-emerald-600 p-3"><div className="text-xs font-bold uppercase">{isID ? "Berhasil diisi" : "Filled"}</div><div className="text-2xl font-black">{importReport.importedCount}</div></div><div className="rounded-xl bg-amber-500/10 text-amber-600 p-3"><div className="text-xs font-bold uppercase">{isID ? "Selisih" : "Mismatch"}</div><div className="text-2xl font-black">{importReport.mismatchCount}</div></div><div className="rounded-xl bg-rose-500/10 text-rose-600 p-3"><div className="text-xs font-bold uppercase">{isID ? "Tidak ditemukan" : "Not found"}</div><div className="text-2xl font-black">{importReport.notFoundCount}</div></div></div><div className="overflow-x-auto mt-4"><table className="premium-table min-w-[850px]"><thead><tr><th>{isID ? "Dokumen / SP2D" : "Document / SP2D"}</th><th>{isID ? "Recipient" : "Recipient"}</th><th>{isID ? "Gross XML" : "Gross XML"}</th><th>{isID ? "Pajak XML" : "Tax XML"}</th><th>{isID ? "Potongan SP2D" : "SP2D Deduction"}</th><th>{isID ? "Selisih" : "Difference"}</th><th>{isID ? "Hasil" : "Result"}</th></tr></thead><tbody>{importReport.groups.map((group) => <tr key={group.documentNumber}><td><div className="font-bold">{group.documentNumber}</div><div className="text-xs text-muted-foreground">{group.spmNumber || (isID ? "SP2D tidak ditemukan" : "SP2D not found")}</div></td><td>{group.recipientCount}</td><td>Rp{money.format(group.xmlGross)}</td><td>Rp{money.format(group.xmlTax)}</td><td>{group.sp2dDeduction === null ? "—" : `Rp${money.format(group.sp2dDeduction)}`}</td><td>{group.difference === null ? "—" : `Rp${money.format(group.difference)}`}</td><td><span className={`badge ${group.status === "IMPORTED" || group.status === "ALREADY_FILLED" ? "badge-completed" : "bg-amber-500/10! text-amber-600!"}`}>{group.status}</span></td></tr>)}</tbody></table></div></section>}
    {ptkpImportReport && <section className="glass-card p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="font-black text-lg">Hasil import Master PTKP</h2><p className="text-sm text-muted-foreground">{ptkpImportReport.fileName} · {ptkpImportReport.totalRows} baris terbaca</p></div><button onClick={() => setPtkpImportReport(null)}><X size={20}/></button></div><div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4"><div className="rounded-xl bg-emerald-500/10 text-emerald-600 p-3"><div className="text-xs font-bold uppercase">Diproses</div><div className="text-2xl font-black">{ptkpImportReport.importedCount}</div></div><div className="rounded-xl bg-amber-500/10 text-amber-600 p-3"><div className="text-xs font-bold uppercase">Duplikat</div><div className="text-2xl font-black">{ptkpImportReport.duplicateCount}</div></div><div className="rounded-xl bg-rose-500/10 text-rose-600 p-3"><div className="text-xs font-bold uppercase">Invalid</div><div className="text-2xl font-black">{ptkpImportReport.invalidCount}</div></div></div><div className="overflow-x-auto mt-4"><table className="premium-table min-w-[1050px]"><thead><tr><th>Baris</th><th>NIK</th><th>Nama</th><th>PTKP</th><th>Tahun</th><th>Kategori</th><th>Sumber</th><th>Hasil</th></tr></thead><tbody>{ptkpImportReport.rows.map((row) => <tr key={`${row.rowNumber}-${row.nik}-${row.taxYear}`}><td>{row.rowNumber}</td><td className="font-mono text-xs">{row.nik}</td><td className="font-bold">{row.name}</td><td>{row.statusPtkp}</td><td>{row.taxYear}</td><td>{row.category || "—"}</td><td>{row.sourceData || "—"}</td><td><span className={`badge ${row.status === "VALID" ? "badge-completed" : row.status === "DUPLICATE" ? "bg-amber-500/10! text-amber-600!" : "bg-rose-500/10! text-rose-600!"}`}>{row.status}{row.message ? ` · ${row.message}` : ""}</span></td></tr>)}</tbody></table></div></section>}
